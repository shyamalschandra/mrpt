...
double feature_extraction_test_Harris(int N, int h)
{
	...
	tictac.Tic();
	for (int i = 0; i < N; i++) fExt.detectFeatures(img, featsHarris);
...
double feature_extraction_test_KLT(int N, int h)
{
	...
	tictac.Tic();
	for (int i = 0; i < N; i++) fExt.detectFeatures(img, featsKLT);
...
double feature_extraction_test_SIFT(int N, int h)
{
	...
	tictac.Tic();
	for (int i = 0; i < N; i++) fExt.detectFeatures(img, featsSIFT);
...
double feature_extraction_test_SURF(int N, int h)
{
	...
	tictac.Tic();
	for (int i = 0; i < N; i++) fExt.detectFeatures(img, featsSURF);
...
GENERATE_BENCHMARK_FASTERS(
	feature_extraction_test_FAST9, detectFeatures_SSE2_FASTER9)
GENERATE_BENCHMARK_FASTERS(
	feature_extraction_test_FAST10, detectFeatures_SSE2_FASTER10)
GENERATE_BENCHMARK_FASTERS(
	feature_extraction_test_FAST12, detectFeatures_SSE2_FASTER12)
